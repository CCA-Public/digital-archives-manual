...
Once your processing spreadsheet is reviewed and approved by the Digital Archivist, use the following instructions to enter the information into TMS, copying and pasting from the spreadsheet when possible. This can be done most efficiently by making use of Templates. Templates exist for file-level description of various types of material already; for most efficient work, create a custom version of the "Archives - Files - Born Digital" model for your archive, containing the following information:  

* Archives creator  
* Fonds number  
* Credit Line

Especially for high-volume collections, using an archive- and type-specific model for data entry will greatly speed up the process. All file-level records should be created and associated with their series and accession in batch.

The only types of information you should enter into TMS that will not be found in your processing spreadsheet are associations and the elements in [Internal management information](#internalmanagement) below (note that many of these are already entered in the existing Archives data entry templates).
...
At CCA, we typically do not process digital files at the item-level. Item-level metadata is instead automatically generated by Archivematica and saved into an `amdSec` in the AIP METS file for each digital file within a SIP. This information is accessible to researchers via SCOPE.

The Digitization team has described individual digital files as part of the Find and Tell project, and maintain their own file-level descriptive standards which are occasionally applied to born-digital archival material. This descriptive work is largely outside of the work of the Archives team.
